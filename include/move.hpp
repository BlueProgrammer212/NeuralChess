...
inline void addMove(int lsf) noexcept { //Add a move to the LSF array.
    if (lsf < 0 || lsf > 63) return;
...
{ 
    ...
    int new_type = (piece_color & Bitboard::Sides::WHITE ?
                    Bitboard::Pieces::Q : Bitboard::Pieces::q);
...
inline void generatePawnCaptures(int lsf)
 {
    ...
    if (!Bitboard::isPawn(pawn)) return;
    ...
    for (int i = direction_offset_start; i < direction_offset_end; ++i) {
        ...
        if (canCapture(dlsf)) { renderMove(dlsf); }
    }
}

inline void generateSlidingMoves(int lsf) {
    int sliding_piece = Globals::bitboard[lsf];
    if (!Bitboard::isSlidingPiece(sliding_piece)) return;    
}
...
inline void renderPseudoLegalMoves(int lsf) 
{
    ...
    if (Globals::bitboard[lsf] == Bitboard::Pieces::e || alliance_value != Globals::side) return;
    ...
    case Bitboard::Pieces::P:        
        for (int i = 1; i < pawn_range; ++i) {
            ...
            if (notEmpty(target_square)) break;
